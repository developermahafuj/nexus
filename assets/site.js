...
function choosePackage(button,{scroll=true}={}){
  ...
  packageButtons.forEach(item=>{
    ...
    if(label) label.textContent=item.dataset.defaultLabel||`Choose ${item.dataset.tier} Package`;
...
teamFilters.forEach(filter=>filter.addEventListener('click',()=>{
  ...
  teamCards.forEach(card=>{
    const show=target==='all'||(card.dataset.department||'').split(/\s+/).includes(target);
